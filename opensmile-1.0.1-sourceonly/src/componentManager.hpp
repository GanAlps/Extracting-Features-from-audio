...
public:
  ...
  cVector* tick(int threadId, long long tickNr);  // call all components to process one time step ('tick')

  // this function calls tick() until 0 components run successfully, returns total number of ticks
  cVector* runSingleThreaded(long long maxtick=-1, long long tickNr = -1);
