...
class DLLEXPORT cSmileComponent {
  ...
  protected:
    ...
    virtual int* myTick1(long long t) { return 0; }
    virtual cVector* myTick2(long long t) { return 0; }
  ...
  public:
    ...
    int* tick1(long long t, int _EOI=0) {
	  int a[3] = {0};
      if (_EOI) {
        if (!isEOI()) {
          setEOI();
          return a; // one "successful" tick to set up all components in EOI state
        }
      }
      if (_isReady) { 
        if (doProfile) {
    ...
    cVector* tick2(long long t, int _EOI=0) {
		cVector* vec = NULL;
      if (_EOI) {
        if (!isEOI()) {
          setEOI();
          return vec; // one "successful" tick to set up all components in EOI state
        }
      }
      if (_isReady) { 
        if (doProfile) {
