...
cVector* cComponentManager::tick(int threadId, long long tickNr)
{
  if (!ready) return 0;
  int i,*a;
  cVector *vec;
  ...
  for (i=0; i<lastComponent; i++) {
    if (component[i] != NULL) {
      if (((threadId == -1)||(threadId == componentThreadId[i]))&&(componentThreadId[i]!=-2)) {
        ...
        if(strcmp(component[i]->getInstName(),"csvSink")==0){
			vec = component[i]->tick2(tickNr,EOI);
			nRun++;
			//printf(" %s =H0= %f\n",vec->name(0),vec->dataF[0]);
          //printf("%s.tick1() executed successfully!",component[i]->getInstName());
          SMILE_DBG(4,"%s.tick() executed successfully!",component[i]->getInstName());
		}
        else if (component[i]->tick(tickNr,EOI)) {
          nRun++;
          //printf("%s.tick() executed successfully!",component[i]->getInstName());
          SMILE_DBG(4,"%s.tick() executed successfully!",component[i]->getInstName());
        }
      }
    }
...
cVector* cComponentManager::runSingleThreaded(long long maxtick,long long tickNr)
{
  int nRun;
  cVector *vec2;
  ...
  if ((nRun<=0) && ((tickNr < maxtick)||(maxtick==-1))) {
