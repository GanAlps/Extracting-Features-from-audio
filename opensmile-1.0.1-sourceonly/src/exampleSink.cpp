...
int cExampleSink::myTick(long long t)
{
  ...
  if (fHandle != NULL) {
    for (i=0; i<vec->N; i++) {
      fprintf(fHandle, "%s = %f \t",vec->name(i),vec->dataF[i]);
    }
    fprintf(fHandle,"I is = %d",i);
  }
  fprintf(fHandle,"\n\n\n");
...
int* cExampleSink::myTick1(long long t)
{
  SMILE_DBG(4,"tick # %i, reading value vector:",t);
  cVector *vec= reader->getFrameRel(lag);  //new cVector(nValues+1);
  int a[3] = {0,1,2};
...
cVector* cExampleSink::myTick2(long long t)
{
  SMILE_DBG(4,"tick # %i, reading value vector:",t);
  cVector *vec= reader->getFrameRel(lag);  //new cVector(nValues+1);
  //int a[3] = {0,1,2};
