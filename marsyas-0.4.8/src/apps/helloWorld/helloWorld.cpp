...
double* MyTick(MarSystem* pnet){
		...
		const realvec& spectrum =
		    pnet->getctrl("Fanout/Fanout1/Series/Spec/Spectrum/spk/mrs_realvec/processedData")->to<mrs_realvec>(); //512 X 1 vector
		for(i=0;i<512;i++)
			FVec[i] = spectrum(i);
	
		const realvec& center = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerCentroid/Centroid/centeroid/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = center(0,0);
		
		const realvec& rolloff = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerRolloff/Rolloff/rolloff/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = rolloff(0,0);
		
		
		////const realvec& flux = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerFlux/Flux/flux/mrs_realvec/processedData")->to<mrs_realvec>();
		////FVec[i++] = flux(0,0); //ERROR: output is nan or 0??
		
		const realvec& Specmean = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerSpecMean/Mean/Specmean/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = Specmean(0,0);
		
		const realvec& Specmed = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerSpecMed/Median/Specmed/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = Specmed(0,0);
		
		//const realvec& mean = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerMean/Mean/mean/mrs_realvec/processedData")->to<mrs_realvec>();
		//FVec[i++] = mean(0,0);
		
		const realvec& StdDev = pnet->getctrl("Fanout/Fanout1/Series/SerStdDev/StandardDeviation/stddev/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = StdDev(0,0);
		
		const realvec& skew = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerSkew/Skewness/skewness/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = skew(0,0);
		
		const realvec& kurtosis = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerKurtosis/Kurtosis/kurtosis/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = kurtosis(0,0);
		
		const realvec& power = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerPower/Power/power/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = power(0,0);
		
		////const realvec& autocor = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerAutoCor/AutoCorrelation/autocor/mrs_realvec/processedData")->to<mrs_realvec>();
		////FVec[i++] = autocor(0,0);
		
		const realvec& zerocross = pnet->getctrl("Fanout/Fanout1/Series/SerZeroCross/ZeroCrossings/zerocross/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = zerocross(0,0);
		
		const realvec& rms = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerRms/Rms/rms/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = rms(0,0);
		
		const realvec& amdf = pnet->getctrl("Fanout/Fanout1/Series/SerAmdf/AMDF/amdf/mrs_realvec/processedData")->to<mrs_realvec>(); //1 X 512 vector
		for(j=0;j<512;j++,i++)
			FVec[i] = amdf(j);
		
		const realvec& powspec = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerMFCC/PowerSpectrum/powspec/mrs_realvec/processedData")->to<mrs_realvec>(); //257 X 1 vector
		for(j=0;j<257;j++,i++)
			FVec[i] = powspec(j);
		
		const realvec& mfcc = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerMFCC/MFCC/mfcc/mrs_realvec/processedData")->to<mrs_realvec>(); //13 X 1 vector
		for(j=0;j<13;j++,i++)
			FVec[i] = mfcc(j);
		
		const realvec& lpc = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerLPC/LPC/lpc/mrs_realvec/processedData")->to<mrs_realvec>(); //12 X 1 vector
		for(j=0;j<12;j++,i++)
			FVec[i] = lpc(j);
		
		const realvec& lpcc = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerLPC/LPCC/lpcc/mrs_realvec/processedData")->to<mrs_realvec>(); //10 X 1 vector
		for(j=0;j<10;j++,i++)
			FVec[i] = lpcc(j);
		
		//const realvec& lsp = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerLPC/LSP/lsp/mrs_realvec/processedData")->to<mrs_realvec>(); //12 X 1 vector
		//for(j=0;j<12;j++,i++)
		//	FVec[i] = lsp(j);
		
		////const realvec& maxmin = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerMaxMin/MaxMin/maxmin/mrs_realvec/processedData")->to<mrs_realvec>();
		////Fvec[i++] = maxmin(0); Fvec[i++] = maxmin(1); 
		
		//const realvec& f0 = pnet->getctrl("Fanout/Fanout1/Series/SerF0analysis/F0Analysis/f0analysis/mrs_realvec/processedData")->to<mrs_realvec>();
		//Error F0Analysis was not recognized
		
		const realvec& chroma = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerChroma/Chroma/chroma/mrs_realvec/processedData")->to<mrs_realvec>(); // 12 X 1 vector
		for(j=0;j<12;j++,i++)
			FVec[i] = chroma(j);
		
		const realvec& energy = pnet->getctrl("Fanout/Fanout1/Series/SerEnergy/Energy/energy/mrs_realvec/processedData")->to<mrs_realvec>();
		FVec[i++] = energy(0,0);
		
		////const realvec& scf = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerSCF/SCF/scf/mrs_realvec/processedData")->to<mrs_realvec>(); //24 X 1 vector
		////for(j=0;j<24;j++,i++)
		////	FVec[i] = scf(j);
		
		////const realvec& sfm = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerSFM/SFM/sfm/mrs_realvec/processedData")->to<mrs_realvec>();
		////for(j=0;j<CHECK-IT;j++,i++) //Check the no. of rows amd col by: cout << sfm
		////	FVec[i] = sfm(j);
		
		////const realvec& snr = pnet->getctrl("Fanout/Fanout1/Series/SerSnr/SNR/snr/mrs_realvec/processedData")->to<mrs_realvec>(); //2 X 1 vector
		////for(j=0;j<2;j++,i++)
		////	FVec[i] = snr(j);
		
		////const realvec& specsnr = pnet->getctrl("Fanout/Fanout1/Series/Spec/Fanout/Fanout2/Series/SerSpecSnr/SpectralSNR/specsnr/mrs_realvec/processedData")->to<mrs_realvec>();
		////FVec[i++] = specsnr(0,0);
